...
static double idx2dbl(int idx) {
  ...
  /* INTERP_TAB_FR_OFFSET: default = 0.5
   * INTERP_TAB_FR_MULT: default = 10.0
   * INTERP_TAB_MIN_EXP: default = -20
   * INTERP_TAB_MAX_EXP: default = 5
 ...
 */
// DEBUG: add print out variable arg to function
// InterpTab *interp_tab_create_1d(double (*f)(double, void *), void *param, FILE *fh) {
...
// DEBUG: pass a file handle to write down the lookup table
// InterpTab *interp_tab_create(double (*f)(double,double, void *), void *param, FILE *fh) {
InterpTab *interp_tab_create(double (*f)(double,double, void *), void *param) {
  ...
  // DEBUG: store matrix of values to file
  // fprintf(fh, "#f(x,y)=z\n");
  for(idx_x = 0; idx_x < INTERP_TAB_N_ENTRIES; idx_x++) {
    x = idx2dbl(idx_x);
    tab->x[idx_x] = x;
    ...
    // DEBUG print the table to file (do not create newline)
    // fprintf(fh, "x=%g,", tab->x[idx_x]);
    ...
    for(idx_y = 0; idx_y < INTERP_TAB_N_ENTRIES; idx_y++) {
      ...
      z = f(x, y, param);
      tab->z[idx_x][idx_y] = z;
      // DEBUG: write each x,y,z trio to file
      // fprintf(fh, "y=%g,z=%g\t", tab->y[idx_y]);
    }
  // fprintf(fh, "\n");
  }
