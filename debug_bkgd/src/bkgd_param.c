...
// DEBUG: pass file path to lookup table constructor to save its contents at startup  
// static void create_intg_tabs(BkgdParam *parm, char *out_dir) {
static void create_intg_tabs(BkgdParam *parm) {
  ...
  // DEBUG: precursor to file handle to write lookup table to file
  // char *tbl_file_path;
  // FILE *tbl_fh;

  // DEBUG: pass file path to write each lookup table to file
  // tbl_file_path = g_strconcat(out_dir, "blk.drv2.lookup.table.txt", NULL);
  // fprintf(stderr, "lookup-table file path: %s\n", tbl_file_path);
  // tbl_fh = fopen(tbl_file_path, "w");

  fprintf(stderr, "creating interp tab for blk 2nd deriv integrals\n");
  b_intg.f.function = &bkgd_drv2_blk_integrand;
  // DEBUG: pass a file path to interpolation table constructor
  // parm->intg_tab_drv2_blk = interp_tab_create(&bkgd_calc_blk_integral, &b_intg, tbl_fh);
  // fclose(tbl_fh);
  parm->intg_tab_drv2_blk = interp_tab_create(&bkgd_calc_blk_integral, &b_intg);

  // DEBUG: pass file path to write each lookup table to file
  // tbl_file_path = g_strconcat(out_dir, "site.drv2.lookup.table.txt", NULL);
  // tbl_fh = fopen(tbl_file_path, "w");
  fprintf(stderr, "creating interp tab for site 2nd deriv integrals\n");
  b_intg.f.function = &bkgd_drv2_site_integrand;
  // parm->intg_tab_drv2_site = interp_tab_create_1d(&bkgd_calc_site_integral, &b_intg, tbl_fh);
  // fclose(tbl_fh);
  parm->intg_tab_drv2_site = interp_tab_create_1d(&bkgd_calc_site_integral, &b_intg);

  // DEBUG: pass file path to write each lookup table to file
  // tbl_file_path = g_strconcat(out_dir, "blk.intg.lookup.table.txt", NULL);
  // tbl_fh = fopen(tbl_file_path, "w");
  fprintf(stderr, "creating interp tab for blk integrals\n");
  b_intg.f.function = &bkgd_blk_integrand;
  // parm->intg_tab_blk = interp_tab_create(&bkgd_calc_blk_integral, &b_intg, tbl_fh);
  // fclose(tbl_fh);
  parm->intg_tab_blk = interp_tab_create(&bkgd_calc_blk_integral, &b_intg);
  
  // DEBUG: pass file path to write each lookup table to file
  // tbl_file_path = g_strconcat(out_dir, "site.intg.lookup.table.txt", NULL);
  // tbl_fh = fopen(tbl_file_path, "w");
  fprintf(stderr, "creating interp tab for site integrals\n");
  b_intg.f.function = &bkgd_site_integrand;
  // parm->intg_tab_site = interp_tab_create_1d(&bkgd_calc_site_integral, &b_intg, tbl_fh);
  // fclose(tbl_fh);
  parm->intg_tab_site = interp_tab_create_1d(&bkgd_calc_site_integral, &b_intg);
  
  // DEBUG: pass file path to write each lookup table to file
  // tbl_file_path = g_strconcat(out_dir, "blk.drv1.lookup.table.txt", NULL);
  // tbl_fh = fopen(tbl_file_path, "w");
  fprintf(stderr, "creating interp tab for blk 1st deriv integrals\n");
  b_intg.f.function = &bkgd_drv1_blk_integrand;
  // parm->intg_tab_drv1_blk = interp_tab_create(&bkgd_calc_blk_integral, &b_intg, tbl_fh);
  // fclose(tbl_fh);
  parm->intg_tab_drv1_blk = interp_tab_create(&bkgd_calc_blk_integral, &b_intg);

  // DEBUG: pass file path to write each lookup table to file
  // tbl_file_path = g_strconcat(out_dir, "site.drv1.lookup.table.txt", NULL);
  // tbl_fh = fopen(tbl_file_path, "w");
  fprintf(stderr, "creating interp tab for site 1st deriv integrals\n");
  b_intg.f.function = &bkgd_drv1_site_integrand;
  // parm->intg_tab_drv1_site = interp_tab_create_1d(&bkgd_calc_site_integral, &b_intg, tbl_fh);
  // fclose(tbl_fh);
  parm->intg_tab_drv1_site = interp_tab_create_1d(&bkgd_calc_site_integral, &b_intg);
...
BkgdParam *bkgd_param_new(Config *config) {
  ...
  /* load approximation parameters. if they are not present in the file, 
   * default values are used and a warning is issued */
  ...
  /* MODIFIED B INTERPOLATOR: check for enforced points flag */
  if(config_get_boolean(config, "ENFORCE_BGI_PTS"))
    parm->enforce_bgi_pts = TRUE;
  else
    parm->enforce_bgi_pts = FALSE;
  ...
  /* create the lookup table and write it to file */
  // DEBUG: pass file path to lookup table constructor to save its contents at startup
  // char *out_dir;
  // out_dir = config_get_str(config, "OUTPUT_DIR");
  // create_intg_tabs(parm, out_dir);
