...
RecRateTable *rectab_from_feats(SeqFeature *orig_sf, long n_orig_sf, 
  ...
  // DEBUG: count how many extra blocks are needed and print to log
  int num_extra = 0;
  ...
  for(i = 0; i < n_sf; i++) {
    if(sf[i].c.start > last_end+1) {
      ...
      rt->n += 1;
      num_extra += 1; // DEBUG: count number of extra blocks needed
    }
    last_end = sf[i].c.end;
  }
  ...
  // DEBUG: print number of extra blocks that were needed
  fprintf(stderr, "%d extra blocks created to fill gaps in genetic map\n", num_extra);
  ...
  // DEBUG: write a new map file from RecTable data
  // char *gfile_path;
  // gfile_path = "/Users/davidmurphy/GoogleDrive/linked_selection/precalc/debug_bmap/chr14.RecTable.gmap.txt";
  // FILE *gfile_fh;
  // gfile_fh = fopen(gfile_path, "w");

  // rt->cur = 0; // reset cur
  // for (i=0; i < rt->n; i++) {
  //   b = &rt->blk[rt->cur];
  //   fprintf(gfile_fh, "%ld %ld %.14f %.14f %.14f\n",
  //     b->start, b->end, b->r_start, b->r_end, b->rate);
  //   rt->cur += 1;
  // }
  // fclose(gfile_fh);
