...
double bkgd_blk_integrand(double t, void *v) {
  ...
  // DEBUG: modify integral to use segment lengths
  // return (f/(1.0-t)) * (1.0/(1.0 + rho*p->r_near) - 1.0/(1.0 + rho*p->r_far));
  return (f/t) * (1.0 / ((1.0 + rho*p->r_near) * (1.0 + rho*p->r_far)));
...
double bkgd_drv1_site_integrand(double t, void *parm) {
  ...
  // DAVID: incorrect sign? is this just meant to be a magnitude at this point?
...
static inline double get_blk_term(ConsBlock *cblk, BkgdPoint *bpoint,
  ...
  if(cblk->end < bpoint->pos) {
    ...
    // cM/Mb = 0 throughout block region -> interpolate for a single site
    if(r_len == 0.0) {
      len = cblk->end - cblk->start + 1;  // length in BASES (correcting for .bed formatting)
    ...
    // block has some span in genetic distance units
    else {
      // DEBUG: re-define calculation of "term" by multiplying by length
    ...
    if(cblk->r > 0.0) {
      ...
      // DEBUG: change to multiplication by number of bases here
      // term /= cblk->r;
      term *= (double)len;
      if(drv1_term) {
...
void bkgd_calc_b(BkgdPoint *bpoint, GList *cons_list, GList *next_cons, 
  ...
  /* DAVID: why is 1st deriv multiplied by 2?
   * this is not compatible with 2nd deriv. 
   * my solution has no factor of 2
   * suggested FIX:
