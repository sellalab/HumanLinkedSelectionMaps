...
PHASE='phase3'
SNPDIR=/ifs/data/c2b2/gs_lab/dam2214/linked_selection/data/snps
...
if [ $CHR ] && [ $POP ] && [ $TOKEN ]; then 
    ...
    IDS=${SNPDIR}/pops/${POP}.${TOKEN}.IDs.txt    
    # input file path /ifs/data/c2b2/gs_lab/dam2214/linked_selection/data/snps/vcf/chr11.phase-3.vcf.gz
    FIN=${SNPDIR}/vcf/${CHR}.${PHASE}.vcf.gz
    # output file path
    FOUT=${SNPDIR}/frq/${POP}/${CHR}.${POP}.${TOKEN}.${PHASE}.frq.count.gz

    # create output file directory if it doesn't exist yet
    if [ ! -d ${SNPDIR}/frq/${POP} ]; then
	mkdir ${SNPDIR}/frq/${POP};
    fi
    ...
    # $rm_mono |
    # filter rare phase-3 duplicate SNPs using awk unique one-liner. gzip high compression mode
    awk '!a[$2]++' | gzip -9 > $FOUT
